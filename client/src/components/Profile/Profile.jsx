...
const Profile = ({onSubmit, onChange, errors, user}) => (
  <Card className={styles.container}>
    <form action="/" onSubmit={onSubmit}>
      <h2 className={styles.card_heading}><TranslationContainer translationKey="user_welcome_text"/> {JSON
          .parse(localStorage.getItem('usrname'))
          .name}</h2>
      <TranslationContainer translationKey="update_password_text"/> {errors.summary && <p className={styles.error_message}>{errors.summary}</p>}
